# 🟢 Scribelia Status

[![Status](https://status.scribelia.com/badge.svg)](https://status.scribelia.com)

> État des services Scribelia en temps réel

**Statut actuel:** 🟢 Tous les systèmes opérationnels

*Dernière mise à jour: 04/02/2026 10:39:36*

---

## 📊 État des services

| Service | État | Temps de réponse | Uptime (24h) | Uptime (7j) | Uptime (30j) |
|---------|------|------------------|--------------|-------------|--------------|
| **API** | ✅ Opérationnel | 670ms | 55.17% | 59.56% | 71.54% |
| **Website** | ✅ Opérationnel | 650ms | 100% | 100% | 99.77% |

---

## 📈 Historique récent

### 🚨 Incidents en cours

- **API** - ❌ Hors service depuis 11h 20m
- **API** - ⚠️ Dégradé depuis 18h 1m
- **API** - ⚠️ Dégradé depuis 1j 21h
- **API** - ❌ Hors service depuis 2j 0h
- **API** - ⚠️ Dégradé depuis 2j 3h
- **API** - ❌ Hors service depuis 3j 15h
- **API** - ⚠️ Dégradé depuis 3j 21h
- **API** - ⚠️ Dégradé depuis 3j 23h
- **API** - ❌ Hors service depuis 4j 2h
- **API** - ❌ Hors service depuis 4j 14h
- **API** - ⚠️ Dégradé depuis 4j 14h
- **API** - ❌ Hors service depuis 4j 15h
- **API** - ❌ Hors service depuis 4j 17h
- **API** - ❌ Hors service depuis 5j 16h
- **API** - ❌ Hors service depuis 5j 17h
- **API** - ⚠️ Dégradé depuis 6j 15h
- **API** - ⚠️ Dégradé depuis 6j 20h
- **API** - ⚠️ Dégradé depuis 9j 16h
- **API** - ❌ Hors service depuis 13j 18h
- **API** - ⚠️ Dégradé depuis 13j 18h
- **API** - ❌ Hors service depuis 22j 19h
- **API** - ⚠️ Dégradé depuis 22j 21h

### ✅ Incidents résolus récemment

- **API** - 04/02/2026 - Résolu en 43s
- **API** - 04/02/2026 - Résolu en 8m
- **API** - 04/02/2026 - Résolu en 1h 11m
- **API** - 04/02/2026 - Résolu en 2h 34m
- **API** - 03/02/2026 - Résolu en 12m

---

## 📋 Services monitorés

### API
- **URL:** `https://api.scribelia.com/health`
- **Description:** API Backend
- **Temps de réponse moyen (24h):** 683ms
- **Temps de réponse moyen (7j):** 674ms

### Website
- **URL:** `https://scribelia.com`
- **Description:** Frontend Application
- **Temps de réponse moyen (24h):** 615ms
- **Temps de réponse moyen (7j):** 339ms

---

## 🔧 À propos

Cette page de statut est mise à jour automatiquement toutes les 5 minutes via GitHub Actions.

- 📁 [Voir les données brutes](./data/status.json)
- 🌐 [Page de statut](https://status.scribelia.com)

---

*Propulsé par [Scribelia Status Monitor](https://github.com/scribelia-org/scribelia-status)*
