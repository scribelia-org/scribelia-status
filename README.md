# 🟢 Scribelia Status

[![Status](https://status.scribelia.com/badge.svg)](https://status.scribelia.com)

> État des services Scribelia en temps réel

**Statut actuel:** 🟢 Tous les systèmes opérationnels

*Dernière mise à jour: 31/01/2026 00:53:13*

---

## 📊 État des services

| Service | État | Temps de réponse | Uptime (24h) | Uptime (7j) | Uptime (30j) |
|---------|------|------------------|--------------|-------------|--------------|
| **API** | ✅ Opérationnel | 604ms | 53.52% | 67.51% | 73.91% |
| **Website** | ✅ Opérationnel | 93ms | 100% | 99.55% | 99.79% |

---

## 📈 Historique récent

### 🚨 Incidents en cours

- **API** - ❌ Hors service depuis 4h 48m
- **API** - ⚠️ Dégradé depuis 5h 9m
- **API** - ❌ Hors service depuis 5h 31m
- **API** - ❌ Hors service depuis 7h 30m
- **API** - ❌ Hors service depuis 1j 6h
- **API** - ❌ Hors service depuis 1j 7h
- **API** - ⚠️ Dégradé depuis 2j 5h
- **API** - ⚠️ Dégradé depuis 2j 10h
- **API** - ⚠️ Dégradé depuis 5j 6h
- **API** - ❌ Hors service depuis 9j 8h
- **API** - ⚠️ Dégradé depuis 9j 8h
- **API** - ❌ Hors service depuis 18j 9h
- **API** - ⚠️ Dégradé depuis 18j 11h

### ✅ Incidents résolus récemment

- **API** - 30/01/2026 - Résolu en 7m
- **API** - 30/01/2026 - Résolu en 7m
- **API** - 30/01/2026 - Résolu en 8m
- **API** - 30/01/2026 - Résolu en 1m
- **API** - 30/01/2026 - Résolu en 4m

---

## 📋 Services monitorés

### API
- **URL:** `https://api.scribelia.com/health`
- **Description:** API Backend
- **Temps de réponse moyen (24h):** 695ms
- **Temps de réponse moyen (7j):** 650ms

### Website
- **URL:** `https://scribelia.com`
- **Description:** Frontend Application
- **Temps de réponse moyen (24h):** 322ms
- **Temps de réponse moyen (7j):** 356ms

---

## 🔧 À propos

Cette page de statut est mise à jour automatiquement toutes les 5 minutes via GitHub Actions.

- 📁 [Voir les données brutes](./data/status.json)
- 🌐 [Page de statut](https://status.scribelia.com)

---

*Propulsé par [Scribelia Status Monitor](https://github.com/scribelia-org/scribelia-status)*
