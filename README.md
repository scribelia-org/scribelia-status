# 🔴 Scribelia Status

[![Status](https://status.scribelia.com/badge.svg)](https://status.scribelia.com)

> État des services Scribelia en temps réel

**Statut actuel:** 🔴 Panne détectée

*Dernière mise à jour: 31/01/2026 11:47:26*

---

## 📊 État des services

| Service | État | Temps de réponse | Uptime (24h) | Uptime (7j) | Uptime (30j) |
|---------|------|------------------|--------------|-------------|--------------|
| **API** | ❌ Hors service | 10003ms | 53.95% | 66.56% | 73.8% |
| **Website** | ✅ Opérationnel | 390ms | 100% | 99.54% | 99.79% |

---

## 📈 Historique récent

### 🚨 Incidents en cours

- **API** - ❌ Hors service depuis 1s
- **API** - ⚠️ Dégradé depuis 23m
- **API** - ❌ Hors service depuis 3h 30m
- **API** - ❌ Hors service depuis 15h 42m
- **API** - ⚠️ Dégradé depuis 16h 4m
- **API** - ❌ Hors service depuis 16h 26m
- **API** - ❌ Hors service depuis 18h 24m
- **API** - ❌ Hors service depuis 1j 17h
- **API** - ❌ Hors service depuis 1j 18h
- **API** - ⚠️ Dégradé depuis 2j 16h
- **API** - ⚠️ Dégradé depuis 2j 21h
- **API** - ⚠️ Dégradé depuis 5j 17h
- **API** - ❌ Hors service depuis 9j 19h
- **API** - ⚠️ Dégradé depuis 9j 19h
- **API** - ❌ Hors service depuis 18j 20h
- **API** - ⚠️ Dégradé depuis 18j 22h

### ✅ Incidents résolus récemment

- **API** - 31/01/2026 - Résolu en 29m
- **API** - 31/01/2026 - Résolu en 11m
- **API** - 31/01/2026 - Résolu en 8m
- **API** - 31/01/2026 - Résolu en 22m
- **API** - 31/01/2026 - Résolu en 11m

---

## 📋 Services monitorés

### API
- **URL:** `https://api.scribelia.com/health`
- **Description:** API Backend
- **Temps de réponse moyen (24h):** 683ms
- **Temps de réponse moyen (7j):** 652ms

### Website
- **URL:** `https://scribelia.com`
- **Description:** Frontend Application
- **Temps de réponse moyen (24h):** 308ms
- **Temps de réponse moyen (7j):** 352ms

---

## 🔧 À propos

Cette page de statut est mise à jour automatiquement toutes les 5 minutes via GitHub Actions.

- 📁 [Voir les données brutes](./data/status.json)
- 🌐 [Page de statut](https://status.scribelia.com)

---

*Propulsé par [Scribelia Status Monitor](https://github.com/scribelia-org/scribelia-status)*
