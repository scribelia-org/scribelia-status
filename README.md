# 🟡 Scribelia Status

[![Status](https://status.scribelia.com/badge.svg)](https://status.scribelia.com)

> État des services Scribelia en temps réel

**Statut actuel:** 🟡 Performance dégradée

*Dernière mise à jour: 31/01/2026 05:44:05*

---

## 📊 État des services

| Service | État | Temps de réponse | Uptime (24h) | Uptime (7j) | Uptime (30j) |
|---------|------|------------------|--------------|-------------|--------------|
| **API** | ⚠️ Dégradé | 9637ms | 52.86% | 67.81% | 73.92% |
| **Website** | ✅ Opérationnel | 214ms | 100% | 99.55% | 99.79% |

---

## 📈 Historique récent

### 🚨 Incidents en cours

- **API** - ⚠️ Dégradé depuis 0s
- **API** - ❌ Hors service depuis 9h 39m
- **API** - ⚠️ Dégradé depuis 10h 0m
- **API** - ❌ Hors service depuis 10h 22m
- **API** - ❌ Hors service depuis 12h 21m
- **API** - ❌ Hors service depuis 1j 11h
- **API** - ❌ Hors service depuis 1j 12h
- **API** - ⚠️ Dégradé depuis 2j 10h
- **API** - ⚠️ Dégradé depuis 2j 15h
- **API** - ⚠️ Dégradé depuis 5j 11h
- **API** - ❌ Hors service depuis 9j 13h
- **API** - ⚠️ Dégradé depuis 9j 13h
- **API** - ❌ Hors service depuis 18j 14h
- **API** - ⚠️ Dégradé depuis 18j 16h

### ✅ Incidents résolus récemment

- **API** - 31/01/2026 - Résolu en 1m
- **API** - 30/01/2026 - Résolu en 7m
- **API** - 30/01/2026 - Résolu en 7m
- **API** - 30/01/2026 - Résolu en 8m
- **API** - 30/01/2026 - Résolu en 1m

---

## 📋 Services monitorés

### API
- **URL:** `https://api.scribelia.com/health`
- **Description:** API Backend
- **Temps de réponse moyen (24h):** 704ms
- **Temps de réponse moyen (7j):** 651ms

### Website
- **URL:** `https://scribelia.com`
- **Description:** Frontend Application
- **Temps de réponse moyen (24h):** 326ms
- **Temps de réponse moyen (7j):** 356ms

---

## 🔧 À propos

Cette page de statut est mise à jour automatiquement toutes les 5 minutes via GitHub Actions.

- 📁 [Voir les données brutes](./data/status.json)
- 🌐 [Page de statut](https://status.scribelia.com)

---

*Propulsé par [Scribelia Status Monitor](https://github.com/scribelia-org/scribelia-status)*
